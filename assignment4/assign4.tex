...
\end{align}
Taking dot product of vectors $\vec{CB}$ and $\vec{CD}$ we get
\begin{align*}
  \vec{CB}.\vec{CD} &= \vec{(B-C)}^T\vec{(D-C)}\\
  &= \vec{(B-A+A-C)}^T\vec{(D-A+A-C)}\\
  &= \vec{((B-A)+(A-C))}^T\vec{((D-A)+(A-C))}
\end{align*}
using \eqref{eq2} we get
\begin{align*}
  \vec{CB}.\vec{CD} &= \vec{((B-A)+(A-C))}^T\vec{((D-A)+(A-C))}\\
...
\begin{align}
  \vec{CB}.\vec{CD} &= -\norm{\vec{A-B}}^2+\norm{\vec{A-C}}^2 = 0
\end{align}
since $\vec{CB}.\vec{CD}$ = 0,therefore BC $\perp$ CD and $\angle{BCD}$ is a right angle.
\end{document}
